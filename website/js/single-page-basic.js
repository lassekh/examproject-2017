...
async function getJson(){
	//Hent json
	let jsonObjekt = await fetch(siteURL+"/wp-json/wp/v2/pages_basic/"+id);
	let page = await jsonObjekt.json();

	console.log(page);

	//document.querySelector(".bg_image").setAttribute("src",page.acf.bg_image.sizes.large);
	...
	document.querySelector("#basic_container .sidebar_title").textContent = page.acf.content_sidebar_title;
	document.querySelector("#basic_container .sidebar_description").innerHTML = page.acf.content_sidebar_description;
}
